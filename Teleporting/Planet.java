	...
	public static int minTotalDistance(Planet[] planets) {
		...
		for (int i = 0; i < planets.length; i++) {
			for (int j = (i + 1); j < planets.length; j++) {
				edges.add(new Edge(planets[i], planets[j], distance(planets[i], planets[j])));
			}
		}
		
//		for (int i = 0; i < planets.length-1; i++) {
//			// take planet i and find the distance between this planet and each other planet *after* it.
